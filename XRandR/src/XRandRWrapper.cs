...
namespace XRandR
{
	...
	public class Tools
	{
		...
		public static T [] PtrToStructureArray<T> (IntPtr ptr,int numElements)
		{
			T [] res = new T [numElements];
			for (int i = 0; i < numElements; i++) {
				IntPtr ptr = new IntPtr (ptr.ToInt64 () + i * Marshal.SizeOf (typeof (T)));
				res [i] = (T) Marshal.PtrToStructure (typeof (T), typeof (T));
			}
			return res;
		}
		...
		public class XErrorException : Exception
		{
			...
			{
				string.Format ("got X error: display: {0} error: {1} ({2}) serial: {3} request: {4} minor: {5}",
					xevent.display,
