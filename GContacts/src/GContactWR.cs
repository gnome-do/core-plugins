...
namespace GContacts
{	
	public static class GContactWR
	{
			...
			try {
				...
				foreach (XmlNode contact in contacts.GetElementsByTagName ("entry")) {
					...
					foreach (XmlNode cont in contact.ChildNodes) {
						...
						case ("gd:postalAddress"): address = cont.InnerText.Replace('\n',' '); break;
						}
					}
					if (!String.IsNullOrEmpty(name) && !String.IsNullOrEmpty(email)) {
						//Console.Error.WriteLine ("{0}, {1}, {2}, {3}", name, email, phone, address);
						...
						emails.Clear ();
						//Reset these to avoid them being added elsewhere.
						phone = address = name = email = null;
					}
				}
				Monitor.Exit (items);
			} catch (Exception e) {
